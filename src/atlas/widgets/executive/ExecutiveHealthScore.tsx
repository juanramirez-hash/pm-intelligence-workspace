...
export type ExecutiveHealthTone =
  | 'healthy'
  | 'attention'
  | 'critical'
  | 'neutral'

export interface ExecutiveHealthScoreProps
  extends Omit<
...
export function ExecutiveHealthScore({
  ...
  return (
    ...
    >
      ...
      <div
        className={[
          'mt-3 flex size-24 items-center justify-center rounded-full border-[8px] shadow-inner',
          styles.ring,
        ].join(' ')}
      >
        <span
      ...
      <span
        className={[
          'mt-3 rounded-full px-3 py-1 text-[0.6875rem] font-semibold',
          styles.badge,
        ].join(' ')}
      >
        {label}
