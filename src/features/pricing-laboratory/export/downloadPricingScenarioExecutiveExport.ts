...
function applyColumnFormats(
  ...
  sheet: PricingScenarioExportSheet,
): void {
  if (!sheet.columnFormats) {
    return
  }

  for (const [columnText, format] of Object.entries(sheet.columnFormats)) {
...
export async function downloadPricingScenarioExecutiveExport(
  payload: PricingScenarioExecutiveExportPayload,
): Promise<void> {
